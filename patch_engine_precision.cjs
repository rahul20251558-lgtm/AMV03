...
content = content.replace(/export function createSeededRandom[\s\S]*?return mulberry32\(hashStr\(seedStr\)\);\n\}/, `export function hashStr(s: string): number {
  let h = 2166136261;
...
// Add new type definitions at top
...
// Rewrite generatePrecisionData
const precisionReplacement = `export function generatePrecisionData(
  productName: string,
  strengthMg: number,
  nominalArea: number,
  ...
  function getAreaAndPct(targetPct: number, sampleWt: number, r: () => number) {
    ...
    if (ctx && ctx.methodType === 'dissolution') {
      areaRaw = (targetPct / 100) * (ctx.LC_mg * 1000) * ctx.A_std / (ctx.C_std! * ctx.V_medium! * ctx.DF! * ctx.P! * ctx.F!);
      const area = Math.round(areaRaw + (r() - 0.5) * (nominalArea * 0.002));
      pct = (area / ctx.A_std) * ctx.C_std! * ctx.V_medium! * ctx.DF! * ctx.P! * ctx.F! * 100 / (ctx.LC_mg * 1000);
      return { area, pct: Number(pct.toFixed(2)) };
    } else if (ctx && ctx.methodType === 'assay') {
      // areaRaw from assayPct = (A_T/A_S) * (W_S*P*F/D_S) * (D_T/W_T) * (AVG_WT/LC) * 100
      areaRaw = targetPct / 100 * ctx.A_std / ( (ctx.W_S! * ctx.P! * ctx.F! / ctx.D_S!) * (ctx.D_T! / sampleWt) * (ctx.AVG_WT! / ctx.LC_mg) );
      const area = Math.round(areaRaw + (r() - 0.5) * (nominalArea * 0.002));
      pct = (area / ctx.A_std) * (ctx.W_S! * ctx.P! * ctx.F! / ctx.D_S!) * (ctx.D_T! / sampleWt) * (ctx.AVG_WT! / ctx.LC_mg) * 100;
      return { area, pct: Number(pct.toFixed(2)) };
    } else if (ctx && ctx.methodType === 'related_substances') {
      // imp% = (A_imp / A_std) * (C_std / C_smp) * (100 / RRF)
      areaRaw = (targetPct / 100) * ctx.A_std / ( (ctx.C_std! / ctx.C_smp!) * (1 / ctx.RRF!) );
      const area = Math.round(areaRaw + (r() - 0.5) * (nominalArea * 0.002));
      pct = (area / ctx.A_std) * (ctx.C_std! / ctx.C_smp!) * (100 / ctx.RRF!);
      return { area, pct: Number(pct.toFixed(2)) };
    }
    // Fallback if ctx is missing (should not happen after full migration)
...
content = content.replace(/export function generatePrecisionData\([\s\S]*?return \{\s*analyst1:[\s\S]*?cumulative:[\s\S]*?\}\s*;\s*\}/, precisionReplacement);
