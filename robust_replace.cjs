...
const endIndex = code.indexOf('  const handleExport = async () => {'); // Find the next function
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  const handleGenerate = async () => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
    }, 50); // Just a tiny delay so the UI loader flashes briefly to acknowledge the click
  };
